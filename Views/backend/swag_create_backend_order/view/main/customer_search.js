...
//
Ext.define('Shopware.apps.SwagCreateBackendOrder.view.main.CustomerSearch', {
     ...
     * @array
     */
    alternateClassName: [ 'SwagCreateBackendOrder.view.main.CustomerSearch' ],
    ...
    requires: [
        ...
        'Ext.XTemplate',
        'Shopware.apps.SwagCreateBackendOrder.model.Customer',
        'Shopware.apps.SwagCreateBackendOrder.store.Customer',
        'Ext.grid.Panel'
    ],
    ...
    multiSelectStore: Ext.create('Ext.data.Store', {
        model: 'Shopware.apps.SwagCreateBackendOrder.model.Customer'
    }),
    ...
    initComponent: function() {
        ...
        if (!(me.customerStore instanceof Ext.data.Store)) {
            me.customerStore = Ext.create('Shopware.apps.SwagCreateBackendOrder.store.Customer');
        }

        // We need to filter the store on loading to prevent to show the first article in the store on startup
        me.dropDownStore = Ext.create('Shopware.apps.SwagCreateBackendOrder.store.Customer', {
            listeners: {
                single: true,
                load: function() {
